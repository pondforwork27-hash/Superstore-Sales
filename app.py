...
# ── Sales Growth %: latest year vs previous year in filtered data ─────────────
_yearly = filtered_df.groupby('Year')['Sales'].sum().sort_index()
if len(_yearly) >= 2:
    _growth_pct = (_yearly.iloc[-1] - _yearly.iloc[-2]) / _yearly.iloc[-2] * 100
    _growth_str = f"{'▲' if _growth_pct >= 0 else '▼'} {abs(_growth_pct):.1f}%"
    _growth_delta = f"{int(_yearly.index[-1])} vs {int(_yearly.index[-2])}"
else:
    _growth_str   = "N/A"
    _growth_delta = "Need 2+ years"

k1,k2,k3,k4 = st.columns(4)
k1.metric("💰 Total Sales",     f"${total_sales:,.0f}")
k2.metric("📦 Total Orders",    f"{total_orders:,}")
k3.metric("🧾 Avg Order Value", f"${avg_order_val:,.0f}")
k4.metric("📈 Sales Growth",    _growth_str, delta=_growth_delta)
...
# ── Rank medals assigned by sales rank (already sorted desc) ─────────────────
_rank_medals = ['🥇', '🥈', '🥉', '4️⃣']
...
for _idx, _row in _all_region_stats.iterrows():
    ...
    _aov      = _sales / _orders if _orders else 0          # avg order value
    _share    = _sales / _grand_total * 100 if _grand_total else 0
    _icon     = _region_meta[_region][0]
    _medal    = _rank_medals[_idx] if _idx < len(_rank_medals) else ''
    _is_act   = _region in st.session_state.sel_region_card
    _check    = "  ✓" if _is_act else ""
    # ── Clever label: medal + region · revenue · ASCII share bar · AOV ───
    _bar_len  = int(_share / 5)                             # 0-20 filled blocks
    _bar      = '█' * _bar_len + '░' * (10 - min(_bar_len, 10))
    _label    = (
        f"{_medal} {_icon}  {_region}{_check}\n"
        f"${_sales:,.0f}\n"
        f"{_bar}  {_share:.0f}%\n"
        f"${_aov:,.0f} avg order"
    )
...
(function() {{
  ...
  function styleAll() {{
    ...
    cards.forEach(function(c) {{
      ...
      s.setProperty('min-height', '145px', 'important');
      s.setProperty('height', 'auto', 'important');
      s.setProperty('width', '100%', 'important');
      s.setProperty('padding', '18px 12px 14px', 'important');
      s.setProperty('font-size', '0.82rem', 'important');
...
if _sel_regions:
    fig_map = px.choropleth(
        ...
        labels={'Sales': 'Total Sales ($)'}, custom_data=['Share']
    )
    fig_map.update_traces(
        hovertemplate=(
            "<b>%{hovertext}</b><br>Total Sales: $%{z:,.0f}<br>"
            "Revenue Share: %{customdata[0]:.1f}%<extra></extra>"
        ),
    ...
    for _sreg in _sel_regions:
        ...
        if not _reg_df.empty:
            ...
            fig_map.add_trace(go.Choropleth(
                locations=_reg_df['State Code'].tolist(), z=_reg_df['Sales'].tolist(),
                locationmode="USA-states", colorscale=_fcol, showscale=False,
                marker_line_color=_bcol, marker_line_width=2.5,
                text=_reg_df['State'].tolist(), customdata=_reg_df[['Share']].values,
                hovertemplate=(
                    "<b>%{text}</b><br>Sales: $%{z:,.0f}<br>"
                    "Revenue Share: %{customdata[0]:.1f}%<br>Region: " + _sreg + "<extra></extra>"
                ),
                name=_sreg,
            ))
else:
    fig_map = px.choropleth(
        all_state_sales, locations='State Code', locationmode="USA-states",
        color='Sales', scope="usa", hover_name='State',
        color_continuous_scale="Blues", labels={'Sales': 'Total Sales ($)'}, custom_data=['Share']
    )
    fig_map.update_traces(
        hovertemplate=(
            "<b>%{hovertext}</b><br>Total Sales: $%{z:,.0f}<br>"
...
if st.session_state.clicked_state:
    ...
    if not _hl.empty:
        fig_map.add_trace(go.Choropleth(
            ...
            showscale=False, marker_line_color="#e94560", marker_line_width=3, hoverinfo='skip',
        ))

_gmf = st.session_state.geo_map_filter
if _gmf:
    if _gmf == 'leader':
        _hl_states = [top_state['State']];  _hl_color = '#4299e1'
    elif _gmf == 'top5':
        _hl_states = state_sales.nlargest(5, 'Sales')['State'].tolist();  _hl_color = '#ed8936'
    elif _gmf == 'above_avg':
        _avg_s = state_sales['Sales'].mean()
        _hl_states = state_sales[state_sales['Sales'] > _avg_s]['State'].tolist();  _hl_color = '#9f7aea'
    elif _gmf == 'weakest':
        _hl_states = [state_sales.sort_values('Sales').iloc[0]['State']];  _hl_color = '#e94560'
    else:
        _hl_states = [];  _hl_color = '#ffffff'

    _hl_df = all_state_sales[all_state_sales['State'].isin(_hl_states)]
    if not _hl_df.empty:
        fig_map.add_trace(go.Choropleth(
            locations=_hl_df['State Code'].tolist(), z=_hl_df['Sales'].tolist(),
            ...
            showscale=False, marker_line_color=_hl_color, marker_line_width=3,
            text=_hl_df['State'].tolist(), customdata=_hl_df[['Share']].values,
...
fig_map.update_layout(
    ...
    coloraxis_colorbar=dict(title="Sales ($)", tickprefix="$"), showlegend=False,
)
...
if map_event and map_event.selection and map_event.selection.get("points"):
    ...
    if clicked_abbrev:
        ...
        if new_state and new_state != st.session_state.clicked_state:
            st.session_state.clicked_state = new_state;  st.rerun()
        elif new_state == st.session_state.clicked_state:
            st.session_state.clicked_state = None;  st.rerun()
...
_ts_name  = top_state['State'];   _ts_sales = top_state['Sales']
_bs_name  = _bottom_state['State']; _bs_sales = _bottom_state['Sales']
...
_pip_html = ''.join([
    ...
    ('#9f7aea' if i < _above_avg_states else 'rgba(255,255,255,0.06)') + ';"></div>'
    for i in range(min(_n_active_states, 30))
])

_geo_html = (
    "<!DOCTYPE html><html><head><meta charset=\'utf-8\'>"
    "<style>body{margin:0;padding:0;background:transparent;font-family:-apple-system,BlinkMacSystemFont,Segoe UI,sans-serif;}"
    ...
    "</style><script>function sendFilter(f){window.parent.postMessage({geoFilter:f},'*');}</script>"
    "</head><body>"
  ...
  <div style="display:grid;grid-template-columns:1.4fr 1fr 1fr 1fr;gap:10px;">
    <div class="geo-card-click" onclick="sendFilter('leader')" style="background:linear-gradient(145deg,#0d2240,#112a50);border:1px solid #1e4a80;border-radius:12px;padding:16px 14px 14px;position:relative;overflow:hidden;color:#4299e1;">
      ...
      <div style="font-size:0.65rem;font-weight:700;text-transform:uppercase;letter-spacing:0.13em;color:#4a6580;margin-bottom:12px;display:flex;align-items:center;gap:5px;"><div style="width:5px;height:5px;border-radius:50%;background:#4299e1;flex-shrink:0;"></div>Revenue Leader</div>
      ...
      <div style="margin-top:14px;display:flex;flex-direction:column;gap:5px;">
        <div style="display:flex;align-items:center;gap:6px;"><span style="color:#4a6580;font-size:0.6rem;width:32px;flex-shrink:0;">Leader</span><div style="flex:1;background:rgba(255,255,255,0.05);border-radius:99px;height:4px;"><div style="width:100%;height:4px;background:linear-gradient(90deg,#4299e1,#90cdf4);border-radius:99px;"></div></div><span style="color:#90cdf4;font-size:0.6rem;width:34px;text-align:right;">{state_share:.1f}%</span></div>
        <div style="display:flex;align-items:center;gap:6px;"><span style="color:#4a6580;font-size:0.6rem;width:32px;flex-shrink:0;">Avg</span><div style="flex:1;background:rgba(255,255,255,0.05);border-radius:99px;height:4px;"><div style="width:{_avg_pct:.0f}%;height:4px;background:rgba(66,153,225,0.35);border-radius:99px;"></div></div><span style="color:#4a6580;font-size:0.6rem;width:34px;text-align:right;">{_avg_share:.1f}%</span></div>
      </div>
    ...
    <div class="geo-card-click" onclick="sendFilter('top5')" style="background:rgba(255,255,255,0.025);border:1px solid #162640;border-radius:12px;padding:16px 14px 14px;position:relative;overflow:hidden;color:#ed8936;">
      <div style="font-size:0.65rem;font-weight:700;text-transform:uppercase;letter-spacing:0.13em;color:#4a6580;margin-bottom:12px;display:flex;align-items:center;gap:5px;"><div style="width:5px;height:5px;border-radius:50%;background:{_conc_color};flex-shrink:0;"></div>Concentration</div>
      ...
      <div style="margin-top:14px;width:70px;height:70px;border-radius:50%;background:conic-gradient({_conc_color} 0% {_top5_share:.0f}%,rgba(255,255,255,0.06) {_top5_share:.0f}% 100%);display:flex;align-items:center;justify-content:center;"><div style="width:50px;height:50px;border-radius:50%;background:#0c1a30;display:flex;align-items:center;justify-content:center;font-size:0.72rem;font-weight:700;color:{_conc_color};">{_top5_share:.0f}%</div></div>
    ...
    <div class="geo-card-click" onclick="sendFilter('above_avg')" style="background:rgba(255,255,255,0.025);border:1px solid #162640;border-radius:12px;padding:16px 14px 14px;position:relative;overflow:hidden;color:#9f7aea;">
      <div style="font-size:0.65rem;font-weight:700;text-transform:uppercase;letter-spacing:0.13em;color:#4a6580;margin-bottom:12px;display:flex;align-items:center;gap:5px;"><div style="width:5px;height:5px;border-radius:50%;background:#9f7aea;flex-shrink:0;"></div>Market Spread</div>
    ...
    <div class="geo-card-click" onclick="sendFilter('weakest')" style="background:rgba(233,69,96,0.04);border:1px solid #3d1020;border-radius:12px;padding:16px 14px 14px;position:relative;overflow:hidden;color:#e94560;">
      <div style="font-size:0.65rem;font-weight:700;text-transform:uppercase;letter-spacing:0.13em;color:#4a6580;margin-bottom:12px;display:flex;align-items:center;gap:5px;"><div style="width:5px;height:5px;border-radius:50%;background:#e94560;flex-shrink:0;"></div>Needs Attention</div>
      ...
      <div style="margin-top:12px;background:rgba(255,255,255,0.05);border-radius:99px;height:3px;overflow:hidden;"><div style="width:{_bs_pct:.1f}%;height:3px;border-radius:99px;background:linear-gradient(90deg,#e94560,#fc8181);"></div></div>
...
with _ghcols[0]:
    if st.button("geo_btn_leader",    key="geo_btn_leader",    use_container_width=True):
        st.session_state.geo_map_filter = None if _gmf == "leader" else "leader";  st.rerun()
with _ghcols[1]:
    if st.button("geo_btn_top5",      key="geo_btn_top5",      use_container_width=True):
        st.session_state.geo_map_filter = None if _gmf == "top5" else "top5";  st.rerun()
with _ghcols[2]:
    if st.button("geo_btn_above_avg", key="geo_btn_above_avg", use_container_width=True):
        st.session_state.geo_map_filter = None if _gmf == "above_avg" else "above_avg";  st.rerun()
with _ghcols[3]:
    if st.button("geo_btn_weakest",   key="geo_btn_weakest",   use_container_width=True):
        st.session_state.geo_map_filter = None if _gmf == "weakest" else "weakest";  st.rerun()
...
if _gmf:
    ...
    with _bc2:
        if st.button("✕ Clear", key="clear_geo_filter", use_container_width=True):
            st.session_state.geo_map_filter = None;  st.rerun()
...
with col2:
    ...
    fig_sun = px.sunburst(_sun_df, path=['Category','Segment'], values='Sales',
...
_ab_dims = {
    ...
    "Segment":"Segment","Ship Mode":"Ship Mode","State":"State",
}
...
def _ab_stats(grp):
    ...
    return dict(total=total, orders=orders, avg_ord=avg_ord, top_cat=top_cat, top_sub=top_sub, top_st=top_st)
    ...
    with col:
        ...
        <div class="insight-card" style="text-align:center;padding:12px 8px;">
          ...
          <div style="display:flex;justify-content:center;gap:20px;margin-top:8px;">
            <div><div style="font-size:0.7rem;color:#90cdf4;margin-bottom:2px;">🔵 {val_a}</div><div style="font-size:1.2rem;font-weight:700;color:#fff;">{fa}</div></div>
            <div style="display:flex;align-items:center;"><div style="font-size:1rem;color:{color};font-weight:700;">{arrow} {abs(d):.1f}%</div></div>
            <div><div style="font-size:0.7rem;color:#e94560;margin-bottom:2px;">🔴 {val_b}</div><div style="font-size:1.2rem;font-weight:700;color:#fff;">{fb}</div></div>
          </div>
        </div>""", unsafe_allow_html=True)
...
fig_ab = go.Figure()
fig_ab.add_trace(go.Scatter(x=ma["Month"], y=ma["Sales"], name=f"🔵 {val_a}", line=dict(color="#4299e1", width=2.5), mode="lines+markers", hovertemplate=f"<b>{val_a}</b><br>%{{x}}<br>Sales: $%{{y:,.0f}}<extra></extra>"))
fig_ab.add_trace(go.Scatter(x=mb["Month"], y=mb["Sales"], name=f"🔴 {val_b}", line=dict(color="#e94560", width=2.5), mode="lines+markers", hovertemplate=f"<b>{val_b}</b><br>%{{x}}<br>Sales: $%{{y:,.0f}}<extra></extra>"))
fig_ab.update_layout(title=dict(text="Monthly Sales — A vs B", font=dict(size=13), x=0.5), xaxis_tickangle=-45, yaxis=dict(tickprefix="$", tickformat=",.0f"), legend=dict(orientation="h", yanchor="bottom", y=-0.35, xanchor="center", x=0.5), plot_bgcolor="rgba(0,0,0,0)", paper_bgcolor="rgba(0,0,0,0)", margin=dict(l=10, r=10, t=40, b=10), height=320)
...
group_a_color = "#4299e1";  group_b_color = "#e94560"

fig_cat_ab = px.bar(ab_cat, x="Category", y="Sales", color="Group", barmode="group",
    color_discrete_map={val_a: group_a_color, val_b: group_b_color},
    labels={"Sales": "Total Sales ($)", "Group": ""}, custom_data=["Order Count", "Group"])
fig_cat_ab.for_each_trace(lambda t: t.update(hovertemplate=(f"<b>{t.name}</b><br><b>%{{x}}</b><br>Sales: $%{{y:,.0f}}<br>Orders: %{{customdata[0]:,}}<extra></extra>")))
fig_cat_ab.update_layout(title=dict(text="Category Breakdown — A vs B", font=dict(size=13, color="white"), x=0.5), showlegend=False, plot_bgcolor="rgba(0,0,0,0)", paper_bgcolor="rgba(0,0,0,0)", yaxis=dict(tickprefix="$", tickformat=",.0f", gridcolor='rgba(128,128,128,0.2)', title=dict(text="Sales ($)", font=dict(color="white")), tickfont=dict(color="white")), xaxis=dict(title="", showticklabels=False, showgrid=False, zeroline=False, showline=False), margin=dict(l=10, r=10, t=40, b=30), height=350, hovermode="closest")
st.plotly_chart(fig_cat_ab, use_container_width=True, key="ab_cat")

st.markdown(f"""<div style="display:flex;justify-content:center;gap:20px;margin-bottom:15px;">
    <div style="display:flex;align-items:center;gap:8px;background:{group_a_color};padding:5px 15px;border-radius:20px;"><span style="color:white;font-weight:600;">🔵 {val_a}</span></div>
    <div style="display:flex;align-items:center;gap:8px;background:{group_b_color};padding:5px 15px;border-radius:20px;"><span style="color:white;font-weight:600;">🔴 {val_b}</span></div>
</div>""", unsafe_allow_html=True)
...
for i, cat in enumerate(categories):
    ...
    with cols[i]:
        st.markdown(f'<div style="background:{color};border-radius:20px;padding:6px 0;text-align:center;margin-top:-15px;margin-bottom:10px;"><span style="color:white;font-weight:600;font-size:0.85rem;">{icon} {cat}</span></div>', unsafe_allow_html=True)
...
for col, cat_name, icon in [(col1,'Furniture','📦'), (col2,'Office Supplies','📎'), (col3,'Technology','💻')]:
    ...
    with col:
        st.markdown(f"""<div style="background:linear-gradient(135deg,#0d1b2a,#1b2a3b);border:1px solid #2d4a6b;border-radius:10px;padding:12px;text-align:center;">
...
st.markdown(f"""<div class="insight-card good"><div class="icon">🏆</div><div class="label">A/B Winner — Total Sales</div><div class="value">{winner}</div>
  <div class="detail"><strong>{winner}</strong> outperforms by <strong>{gap_pct:.1f}%</strong> (${winner_tot:,.0f} vs ${loser_tot:,.0f}). Top category: <strong>{sa["top_cat"] if winner == val_a else sb["top_cat"]}</strong> · Top sub-category: <strong>{sa["top_sub"] if winner == val_a else sb["top_sub"]}</strong> · Strongest state: <strong>{sa["top_st"] if winner == val_a else sb["top_st"]}</strong>.</div>
</div>""", unsafe_allow_html=True)
...
with col3:
    ...
    fig_line = px.line(monthly_sales, x='Month', y='Sales', markers=True, text='label', labels={'Sales':'Total Sales ($)','Month':''})
    fig_line.update_traces(line_color='#4299e1', line_width=2.5, marker=dict(size=7, color='#4299e1'), textposition='top center', textfont=dict(size=10, color='#90cdf4'), hovertemplate="<b>%{x}</b><br>Sales: $%{y:,.0f}<extra></extra>")
    fig_line.update_layout(xaxis_tickangle=-45, yaxis=dict(tickprefix="$", tickformat=",.0f"), yaxis_range=[0, monthly_sales['Sales'].max() * 1.18])
...
with col4:
    ...
    fig_subcat = px.bar(subcat_sorted, x='Sub-Category', y='Sales', color='Sales', color_continuous_scale='Teal', labels={'Sales':'Total Sales ($)'})
...
fig_grouped = px.bar(region_seg, x='Region', y='Sales', color='Segment', barmode='group', color_discrete_sequence=px.colors.qualitative.Set2, labels={'Sales':'Total Sales ($)'})
fig_grouped.for_each_trace(lambda t: t.update(hovertemplate=f"<b>%{{x}}</b><br>Segment: {t.name}<br>Sales: $%{{y:,.0f}}<extra></extra>"))
...
with _fc_col2:
    _fc_months = st.selectbox("Forecast horizon", [3, 6, 12], index=1, key="fc_months", format_func=lambda x: f"{x} months")
...
if _fc_dim == "Total":
    ...
    _series_dict = {cat: grp.groupby('Month_dt')['Sales'].sum() for cat, grp in _fc_df.groupby('Category')}
else:
    _series_dict = {reg: grp.groupby('Month_dt')['Sales'].sum() for reg, grp in _fc_df.groupby('Region')}

def _forecast_series(series, n_months):
    series = series.sort_index().asfreq('MS', fill_value=0)
    if len(series) < 4: return None, None, None
    y = series.values.astype(float);  x = np.arange(len(y))
    coeffs = np.polyfit(x, y, 1);  trend = np.poly1d(coeffs)
    detrended = y - trend(x);  months = series.index.month
    ...
    lower = np.maximum(forecast - sigma, 0);  upper = forecast + sigma
    return series, pd.Series(forecast, index=future_dates), (pd.Series(lower, index=future_dates), pd.Series(upper, index=future_dates))

_dim_colors = {"Total":"#4299e1","Furniture":"#48bb78","Office Supplies":"#ed8936","Technology":"#9f7aea","West":"#48bb78","East":"#4299e1","Central":"#ed8936","South":"#9f7aea"}

with _fc_col1:
    fig_fc = go.Figure()
    _fc_total_next = 0;  _fc_growth_pcts = [];  _hist = None
    for _dim_name, _dim_series in _series_dict.items():
        ...
        if _hist is None: continue
        fig_fc.add_trace(go.Scatter(x=_hist.index, y=_hist.values, name=f"{_dim_name} (actual)", line=dict(color=_color, width=2.5), mode="lines+markers", marker=dict(size=5), hovertemplate=f"<b>{_dim_name}</b><br>%{{x|%b %Y}}<br>Actual: $%{{y:,.0f}}<extra></extra>"))
        fig_fc.add_trace(go.Scatter(x=[_hist.index[-1], _fc_vals.index[0]], y=[_hist.values[-1], _fc_vals.values[0]], line=dict(color=_color, width=2, dash='dot'), showlegend=False, hoverinfo='skip', mode='lines'))
        fig_fc.add_trace(go.Scatter(x=_fc_vals.index, y=_fc_vals.values, name=f"{_dim_name} (forecast)", line=dict(color=_color, width=2.5, dash='dash'), mode="lines+markers", marker=dict(size=6, symbol='diamond'), hovertemplate=f"<b>{_dim_name} forecast</b><br>%{{x|%b %Y}}<br>$%{{y:,.0f}}<extra></extra>"))
        _lo, _hi = _ci
        fig_fc.add_trace(go.Scatter(x=list(_fc_vals.index)+list(_fc_vals.index[::-1]), y=list(_hi.values)+list(_lo.values[::-1]), fill='toself', fillcolor=f"rgba{tuple(int(_color.lstrip('#')[i:i+2], 16) for i in (0,2,4))+(0.12,)}", line=dict(color='rgba(0,0,0,0)'), showlegend=False, hoverinfo='skip', name=f"{_dim_name} CI"))
    ...
    fig_fc.update_layout(title=dict(text=f"Sales Forecast — Next {_fc_months} Months", font=dict(size=14), x=0.5), xaxis=dict(title="", tickformat="%b %Y", tickangle=-30), yaxis=dict(tickprefix="$", tickformat=",.0f", gridcolor="rgba(128,128,128,0.15)"), plot_bgcolor="rgba(0,0,0,0)", paper_bgcolor="rgba(0,0,0,0)", legend=dict(orientation="h", yanchor="bottom", y=-0.35, xanchor="center", x=0.5, font=dict(size=10)), margin=dict(l=10, r=10, t=45, b=10), hovermode="x unified", height=420)
...
with _fc_sum_cols[0]:
    st.markdown(f'<div class="insight-card good"><div class="icon">🔮</div><div class="label">Projected Revenue (next {_fc_months}mo)</div><div class="value">${_fc_total_next:,.0f}</div><div class="detail">Based on linear trend + seasonal pattern from filtered data.</div></div>', unsafe_allow_html=True)
with _fc_sum_cols[1]:
    st.markdown(f'<div class="insight-card {_growth_cls}"><div class="icon">📈</div><div class="label">Projected Growth vs Prior Period</div><div class="value">{_growth_arrow} {abs(_avg_growth):.1f}%</div><div class="detail">Compared to the previous {_fc_months} months of actual sales.</div></div>', unsafe_allow_html=True)
with _fc_sum_cols[2]:
    _best_dim = max(_series_dict.keys(), key=lambda k: _forecast_series(_series_dict[k], _fc_months)[1].sum() if _forecast_series(_series_dict[k], _fc_months)[1] is not None else 0)
    st.markdown(f'<div class="insight-card"><div class="icon">🏆</div><div class="label">Highest Forecast Contributor</div><div class="value">{_best_dim}</div><div class="detail">Expected to lead in revenue over the forecast window.</div></div>', unsafe_allow_html=True)

st.markdown("---")

# ── CITIES TABLE ──────────────────────────────────────────────────────────────
st.header("🏙️ Top Cities by Sales")
_agg = filtered_df.groupby(['City', 'State']).agg(**{
    'Total Sales': ('Sales','sum'), 'Orders':('Order ID','nunique'), 'Customers':('Customer ID','nunique'),
}).reset_index()
...
st.dataframe(
    _agg.style.format({'Total Sales':'${:,.2f}','Avg Order':'${:,.2f}','Orders':'{:,}','Customers':'{:,}'}),
